...
class Uploader
{
	...
	function uploadFile( array $fileBrowse = []){
		$result =   false;
	...
	function customizeUploadFile( array $fileBrowse = []){
		$result =   false;
		$size   =   $fileBrowse["size"][0];
		$name   =   $fileBrowse["name"][0];
		$ext    =   $this->getExtension($name);
		...
		else if($this->allowAll || (!$this->allowAll && in_array($ext,$this->extensions)))
		{
			...
			if(move_uploaded_file($fileBrowse["tmp_name"][0], $this->destinationPath.$this->uploadName))
			{
				$result =   true;
