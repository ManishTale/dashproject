...
function comment_form( $args = array(), $post_id = null ) {
	...
	$fields   = array(
		'author' => '<p class="comment-form-author form-group float-label">' . 
										'<input id="author" name="author" class="form-control" placeholder="Name" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" aria-required="true" required />'.
										'<label for="author" class="label">' . esc_html__( 'Name', 'woocommerce' ) . ' <span class="required">*</span></label> ' .										
										'</p>',
		'email'  => '<p class="comment-form-email form-group float-label">' .
										'<input id="email" name="email" class="form-control" placeholder="Email"  type="email" value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="30" aria-required="true" required />'.
										'<label for="email" class="label">' . esc_html__( 'Email', 'woocommerce' ) . ' <span class="required">*</span></label> '.
										'</p>',
		'url'    => '<p class="comment-form-url  form-group float-label">' .
					 '<input id="url" name="url" ' . ( $html5 ? 'type="url"' : 'type="text"' ) . ' class="form-control" placeholder="Website" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" maxlength="200" />'.
					 '<label for="url" class="label">' . __( 'Website' ) . '</label> </p>',
	);

	if ( has_action( 'set_comment_cookies', 'wp_set_comment_cookies' ) && get_option( 'show_comments_cookies_opt_in' ) ) {
		$consent           = empty( $commenter['comment_author_email'] ) ? '' : ' checked="checked"';
		$fields['cookies'] = '<div class="comment-form-cookies-consent"><input class="styled-checkbox" id="wp-comment-cookies-consent" name="wp-comment-cookies-consent" type="checkbox" value="yes"' . $consent . ' />' .
							 '<label for="wp-comment-cookies-consent" class="col-gray f-normal">' . __( 'Save my name, email, and website in this browser for the next time I comment.' ) . '</label></div><br />';
	...
	$defaults = array(
		'fields'               => $fields,
		'comment_field'        => '<p class="comment-form-comment form-group float-label"><textarea id="comment" name="comment" class="form-control" placeholder="Your Review"  cols="45" rows="8" aria-required="true" required></textarea>'.
