...
.product-price,.product-stock-status{ text-align:left !important; width:30% !important;}
td{font-size:16px;font-style:normal;}
th span{font-size:16px;font-weight:bold;font-style:normal;}
.shoppingcartpage{padding-top:0px !important;}
    </style>
...
<form id="yith-wcwl-form" action="<?php echo $form_action ?>" method="post" class="woocommerce">
    ...
    ?>
        <div class="wishlist-title <?php echo ( $is_custom_list ) ? 'wishlist-title-with-form' : ''?>">
            <?php echo apply_filters( 'yith_wcwl_wishlist_title', '' ); ?>
    ...
    <!-- WISHLIST TABLE -->
<section class="shoppingcartpage">
        <div class="container">
          <div class="carttable">
          	<div class="table-responsive">

	<table class="table shop_table cart wishlist_table" data-pagination="<?php echo esc_attr( $pagination )?>" data-per-page="<?php echo esc_attr( $per_page )?>" data-page="<?php echo esc_attr( $current_page )?>" data-id="<?php echo $wishlist_id ?>" data-token="<?php echo $wishlist_token ?>">
        ...
        <tr>
	        ...
	        <?php if( $is_user_owner ): ?>
		        <th class="product-remove"><b>Delete</b></th>
	        <?php
	            $column_count ++;
	        endif;
	        ?>
            ...
            foreach( $wishlist_items as $item ) :
                ...
                if( $product && $product->exists() ) :
                    ...
                    <tr id="yith-wcwl-row-<?php echo $item['prod_id'] ?>" data-row-id="<?php echo $item['prod_id'] ?>">
                        ...
                        <td class="productname product-name">
                        ...
                        <?php if( $show_price ) : ?>
                            <td class="col-primary f-medium product-price">
                                <?php
                    ...
                    <?php if( $is_user_owner ): ?>
                        <td class="product-remove">
                            <div>
                                <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="text-danger" title="<?php echo apply_filters( 'yith_wcwl_remove_product_wishlist_message_title',__( 'Remove this product', 'yith-woocommerce-wishlist' )); ?>"><span class="icon fa fa-trash"></span></a>
                            </div>
                        </td>
                        <?php endif; ?>
                    </tr>
