...
class e_designer_setting
{
	...
	*/
	...
	/*
	* Verify purchased code of Envato
	 */
	public function verify_code()
	{
		global $P9f;
		$settings 		= $P9f->settings;
		if(isset($_POST['purchased_code']) && $_POST['purchased_code'] != '')
		{
			$purchased_code 	= str_replace(' ', '', $_POST['purchased_code']);
			$options 		= array('purchased_code' => $purchased_code);
			$values 		= tshirtecommerce_settings_purchase_code($options);
			if(isset($values['verified_code']) && $values['verified_code'] == 1)
			{
				...
				wp_redirect(admin_url('index.php?page=tshirtecommerce-setting&step=product'));
				exit;
			}
			else
			{
				$error = 'Your purchased code incorrect. Please check your purchased code and try again.'; 
			}
		}
		else
		{
			$error = 'Please add your purchased code.'; 
		}
		$purchased_code 	= '';
		if(isset($settings['purchased_code']))
		{
			$purchased_code = $settings['purchased_code'];
		}
