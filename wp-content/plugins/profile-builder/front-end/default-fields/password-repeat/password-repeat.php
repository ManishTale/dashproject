...
function wppb_password_repeat_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ){
	...
	if ( $form_location != 'back_end' ){
		...
		<p class="form-group float-label  form-row-wide">
			...
			<input class="text-input form-control'. apply_filters( 'wppb_fields_extra_css_class', '', $field ) .'" name="passw2" maxlength="'. apply_filters( 'wppb_maximum_character_length', 70 ) .'" type="password" id="passw2" value="" autocomplete="off" '. $extra_attr .'placeholder="Repeat Password"/><label for="passw2" class="label">' . $item_title.$error_mark . '</label></p>';
