...
function wppb_username_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ){	
	...
	if ( $form_location != 'back_end' ){
        ...
        <p class="form-group float-label  form-row-wide">
			...
			<input class="text-input default_field_username input  form-control'. apply_filters( 'wppb_fields_extra_css_class', '', $field ) .'" name="username" maxlength="'. apply_filters( 'wppb_maximum_character_length', 70 ) .'" type="text" placeholder="User Name" id="username" value="'. esc_attr( $input_value ) .'" '.$readonly.' '. $extra_attr .'/>	<label for="username" class="label">'.$item_title.$error_mark.'</label></p>';
