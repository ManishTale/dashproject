...
function wppb_password_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ){
	...
	if ( $form_location != 'back_end' ){
		...
		<p class="form-group float-label  form-row-wide">
			...
			<input class="text-input form-control '. apply_filters( 'wppb_fields_extra_css_class', '', $field ) .'" name="passw1" maxlength="'. apply_filters( 'wppb_maximum_character_length', 70 ) .'" type="password" id="passw1" value="" autocomplete="off" '. $extra_attr .' placeholder="Password"/> <label for="passw1" class="label">' . $item_title.$error_mark . '</label></p>';
