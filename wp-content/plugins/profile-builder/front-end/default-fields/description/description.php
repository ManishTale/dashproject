...
function wppb_description_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ){	
	...
	if ( $form_location != 'back_end' ){
        ...
        $output = '
		<p class="form-group float-label  form-row-wide">
			<textarea rows="'.$field['row-count'].'" name="description" maxlength="'. apply_filters( 'wppb_maximum_character_length', '', $field ) .'" class="default_field_description form-control"'. apply_filters( 'wppb_fields_extra_css_class', '', $field ) .'" id="description" wrap="virtual" '. $extra_attr .'placeholder="Design Experience">'. esc_textarea( wp_unslash( $input_value ) ).'</textarea><label for="description" class="label">'.$item_title.$error_mark.'</label></p>';
 ...
 <script>
        $(document).ready(function() {
            $('textarea').click(function() {
                $('textarea').parent().addClass("is-active");
