...
function wppb_email_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ){	
	...
	if ( $form_location != 'back_end' ){
		...
		<p class="form-group float-label  form-row-wide">
			...
			<input class="text-input default_field_email input  form-control '. apply_filters( 'wppb_fields_extra_css_class', '', $field ) .'" name="email" maxlength="'. apply_filters( 'wppb_maximum_character_length', 70 ) .'" type="email" id="email" value="'. esc_attr( $input_value ) .'" '. $extra_attr .' placeholder="E-mail" />	<label for="email" class="label">'.$item_title.$error_mark.'</label></p>';
