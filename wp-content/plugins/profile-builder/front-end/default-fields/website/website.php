...
function wppb_website_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ){	
	...
	if ( $form_location != 'back_end' ){
		...
		<p class="form-group float-label  form-row-wide">	
			<input class="text-input default_field_website form-control'. apply_filters( 'wppb_fields_extra_css_class', '', $field ) .'" name="website" maxlength="'. apply_filters( 'wppb_maximum_character_length', 70 ) .'" type="number" id="website" value="'.esc_url( wp_unslash( $input_value ) ).'" '. $extra_attr .' placeholder="Account Number"/><label for="website" class="label">'.$item_title.$error_mark.'</label></p>';
