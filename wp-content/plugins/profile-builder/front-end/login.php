...
function wppb_login_form( $args = array() ) {
    ...
    <div class="col-md-6">
		<form name="' . $args['form_id'] . '" id="' . $args['form_id'] . '" action="'. esc_url( wppb_curpageurl() ) .'" method="post">
			' . $login_form_top . '
			<p class="login-username form-group float-label  form-row-wide">
						<input type="text" name="log" id="' . esc_attr( $args['id_username'] ) . '" class="input  form-control" value="' . esc_attr( $args['value_username'] ) . '" size="20" placeholder="Username or email" />
						<label class="label"  for="' . esc_attr( $args['id_username'] ) . '">' . esc_html( $args['label_username'] ) . '</label>
						</p>
		
			<p class="login-password form-group float-label  form-row-wide">
			<input type="password" name="pwd" id="' . esc_attr( $args['id_password'] ) . '" class="input  form-control" value="" size="20" placeholder="Password"/>
				<label class="label" for="' . esc_attr( $args['id_password'] ) . '">' . esc_html( $args['label_password'] ) . '</label>
...
function wppb_front_end_login( $atts ){
	...
	if( !is_user_logged_in() ){
		...
		// build our form
		$login_form .= '<div id="wppb-login-wrap" class="wppb-user-forms container">';
		...
		$logged_in_message = '<p class="wppb-alert container">';
