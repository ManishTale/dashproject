			...
			foreach ( $wcfm_messages as $wcfm_message ) {
				...
				if( $message_status == 'unread' ) {
					if( ( !wcfm_is_vendor() && $wcfm_message->message_type == 'verification' ) ) {
						...
						$actions = '<a class="wcfm_messages_mark_read wcfm-action-icon" href="#" data-messageid="' . $wcfm_message->ID . '"><span class="wcfmfa fa-check-circle text_tip" data-tip="' . esc_attr__( 'Mark Read', 'wc-frontend-manager' ) . '"></span></a>';
					}
				}
