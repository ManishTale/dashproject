...
global $product, $post, $yith_wcwl, $wpdb;
...
<form class="variations_form cart" method="post" style="display:none" enctype='multipart/form-data' data-product_id="<?php echo $post->ID; ?>" data-product_variations="<?php echo esc_attr( json_encode( $available_variations ) ) ?>">
		...
		<div class="single_variation_wrap" style="display:none;">
			...
			<div class="variations_button">
				...
				$sql = $wpdb->get_results("SELECT COUNT(*) as `cnt` FROM `{$wpdb->yith_wcwl_items}` WHERE `prod_id` = ".$product->id." AND `user_id` = ".get_current_user_id()."");
					
				$wishlistcount = $sql[0]->cnt;

				if(get_current_user_id() == '0')
				{
					?>
												<button type="submit" class="single_add_to_cart_button button alt btn btn-primary"><span class="icon fa fa-shopping-basket"></span> <?php echo $product->single_add_to_cart_text(); ?></button>
							<div class="wishlistproduct" style="margin-left:20px;margin-right:20px;">
													<?php   echo '<a href='.get_home_url().'/my-account/'.'>';?>
				  ...
				  <!-- <div class="shareproduct dropdown">
