  ...
  foreach ( $raw_zones as $raw_zone ) {
		...
		if ( in_array( 'wcfmmp_product_shipping_by_zone', $methods_id ) ) {
			...
			if ( !empty( $shipping_methods ) ) {
				echo '<br /><div class="wcfmmp_shipment_rules_display"><span class="wcfmmp-store-shipping-rule" style="text-decoration:underline;">'. __( $zone->get_zone_name() . ' Shipping Rules:' , 'wc-multivendor-marketplace') . '</span></div><br />'; 
          ...
          if ( $method['id'] == 'flat_rate' ) {
					  ...
					  if( $setting_cost ) {
						  echo '<div class="wcfmmp_shipment_rules_display"><span class="wcfmmp-store-shipping-rule" style="color:#111;">'. __( 'Flat Rate', 'wc-multivendor-marketplace' ) . '</span>: ' . wc_price( $setting_cost ) .'</div>';
					  }
          } elseif( $method['id'] == 'free_shipping' ) {
          	$min_amount = ! empty( $method['settings']['min_amount'] ) ? $method['settings']['min_amount'] : 0;
          	if( $min_amount ) {
          		echo '<div class="wcfmmp_shipment_rules_display"><span class="wcfmmp-store-shipping-rule" style="color:#111;">'. __( 'Free Shipping', 'wc-multivendor-marketplace' ) . '</span>: ' . sprintf( __ ('Available for shopping more than <b>%s%d</b>.', 'wc-multivendor-marketplace'), get_woocommerce_currency_symbol(), $min_amount ) .'</div>';
          	}
          } elseif( $method['id'] == 'local_pickup' ) {
          	echo '<div class="wcfmmp_shipment_rules_display"><span class="wcfmmp-store-shipping-rule" style="color:#111;">'. __( 'Local Pickup', 'wc-multivendor-marketplace' ) . '</span>: ' . __ ('Available', 'wc-multivendor-marketplace');
          	$setting_cost = isset( $method['settings']['cost'] ) ? stripslashes_deep( $method['settings']['cost'] ) : '';
					  if( $setting_cost ) {
